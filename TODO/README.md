...
| --- | --- | --- | --- | --- |
| 2 | [Citation `[N]` hover URL tooltip](citation-hover-url-tooltip.md) | On hover over a `[N]` bracket, show the source URL in an overlay so the user can judge source type without scrolling. | [x] | — |
| 3 | [Citation `[N]` click flash-highlights source](citation-click-highlight-source.md) | When the user clicks `[N]`, scroll to the source AND briefly fade-in/fade-out a primary-tinted background on the target row (~3s) so it's easy to spot in long lists. | [x] | — |
