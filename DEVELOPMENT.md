...
The hard split: **pages are server-side, views are client-side.** Pages own everything that touches the server (data fetching, auth, route control flow, server→client mapping). Views own everything that runs in the browser (JSX, hooks, event handlers, realtime subscriptions). There is no in-between — a `.view.tsx` never `await`s a server function, and a `page.tsx` never renders markup.

- **Page** (`app/**/page.tsx`) — a server component shell. Resolves route params, fetches all initial data via commands/repositories, performs auth (`getCurrentUserId()`, `requireAuth()`), runs control flow (`notFound()`, `redirect()`), maps server rows into the shape the view will consume (e.g. building an `initialTrustMap` from `findSourceTrustByUrls(...)` rows), and renders a single view component. **No markup, no `<div>`, no Tailwind classes** — control flow (`if (!x) notFound()`) and multiple `await`s are fine. Pages may render multiple branches as long as each branch is a single component, not JSX.
- **View** (`*.view.tsx`) — **always declares `'use client'` at the top of the file**, even if no hooks are currently used (consistency + future-proofing). Sync function component (never `async`). Receives all data as props from the page; **no value imports from `@/server/*`** and no server function calls during render (`import type { … } from '@/server/…'` is fine — types are erased at compile time). Primarily JSX. **At most one `.view.tsx` per feature folder** — the entry point called from the page. May contain trivial UI state (a disclosure toggle, a controlled input), small inline onClick wiring, and pure helper functions. **Should NOT contain substantial logic** — see below.

### View prop shape — pass DB rows directly

Views consume Supabase row types (`Database['public']['Tables']['x']['Row']`) directly when the row's shape matches what the JSX needs. Don't invent intermediate DTO files. If a transformation is required (joining two reads, building a lookup map, normalizing a JSON column), do it inline in the page before rendering the view — not in a `*.dto.ts` and not in the view itself.
...
- Pages (`app/**/page.tsx`) contain no markup — only data resolution + a single view render. Layouts may have markup. Pages own all server-side work: data fetching, auth (`getCurrentUserId()`, `requireAuth()`), control flow (`notFound()`, `redirect()`), and server→client mapping (e.g. building an `initialTrustMap` from row fetches).
- Views (`*.view.tsx`) **always declare `'use client'` at the top of the file**, with no exceptions — even static views with no hooks. Views are **synchronous function components** (never `async`).
- Views have **no value imports from `@/server/*`** and never call server functions during render. All server data arrives as props from the page. Type-only imports (`import type { … } from '@/server/…'`) are allowed — types are erased at compile time and don't pull server code into the client bundle.
- Views accept Supabase row types directly (`Database['public']['Tables']['x']['Row']`) when the row matches what the JSX needs — no intermediate DTO files. Any transformation (joins, lookup maps, normalization) happens inline in the page.
