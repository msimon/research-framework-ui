...
type StepInterviewInput = {
  userId: string;
  subjectId: string;
};

type StepInterviewResult = {
  turn: TurnRow;
  step: AgentStep;
  done: boolean;
};
...
export async function saveInterviewAnswer(subjectId: string, params: { turnId: string; answer: string }) {
...
async function runInitInterviewStep(input: StepInterviewInput): Promise<StepInterviewResult> {
